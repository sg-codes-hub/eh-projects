...
function normalize(q, source) {
  ...
  copy.module = String(copy.module || (
    copy.book === 'First Flight' && copy.chapter && poetryChapters.has(copy.chapter)
      ? 'First Flight Poetry'
...
const base = readJson(path.join(DATA, 'questions.json'));
const baseQuestions = Array.isArray(base.questions)
  ? base.questions.map(q => normalize(q, 'questions.json')) : [];

const manifest = readJson(path.join(BANKS, 'manifest.json'));
...
for (const q of [...baseQuestions, ...extras]) {
  const key = q.id
    ? `id:${q.id}`
    : `text:${String(q.question || q.prompt || '').trim().toLowerCase()}|marks:${q.marks}|chapter:${q.chapter || ''}`;
...
const overrideFiles = [
  'model-answer-quality-overrides.json',
  'model-answer-quality-overrides-lit-01.json',
  'model-answer-quality-overrides-lit-02.json',
  'model-answer-quality-overrides-lit-03.json',
  'model-answer-quality-overrides-lit-04.json',
  'model-answer-quality-overrides-lit-05.json',
  'model-answer-quality-overrides-lit-06.json',
  'model-answer-quality-overrides-lit-07.json',
  'model-answer-quality-overrides-lit-08.json',
...
const output = {
  course: 'Class 10 First Language English (15-E)',
  academic_year: '2026-27',
  bank_version: `Merged production bank • manifest ${manifest.version || 'unknown'}`,
  generated_from: ['data/questions.json', 'data/banks/manifest.json', ...bankNames.map(n => `data/banks/${n}`), ...overrideFiles.map(n => `data/${n}`)],
  question_count: merged.length,
  questions: merged
};

fs.writeFileSync(OUTPUT, JSON.stringify(output, null, 2) + '\n', 'utf8');
console.log(`Generated ${OUTPUT} with ${merged.length} questions.`);
