...
(function () {
  ...
  function publish(questions, meta) {
    window.EnglishHubQuestions = questions;
    window.qs = questions;
    window.EnglishHubBankStatus = meta;
    return questions;
  }
  async function loadMerged() {
    const response = await originalFetch(MERGED_PATH, { cache: 'no-store' });
  ...
  async function loadLegacy() {
    ...
    for (const q of [...baseQuestions, ...extras]) {
      ...
      if (seen.has(key)) continue; seen.add(key); merged.push(q);
    }
    ...
    for (const overrides of overrideResults) for (const q of merged) {
      const o = overrides[q.id]; if (!o) continue;
  ...
  async function loadAll() {
    ...
    readyPromise = loadMerged().catch(() => loadLegacy());
  ...
  window.fetch = async function (input, init) {
    ...
    try {
      ...
      return new Response(JSON.stringify({ course: 'Class 10 First Language English (15-E)', academic_year: '2026-27', bank_version: 'Merged production bank', questions: merged }), { status: 200, headers: { 'Content-Type': 'application/json' } });
