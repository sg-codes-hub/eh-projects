...
(function(){
  ...
  const isRewrite=q=>/rewrite|reported speech|indirect speech|active.*passive|passive.*active|degree|transformation|as directed|sentence transformation/.test(meta(q)+' '+text(q));
  ...
  function make(seed){return withSeed(seed,()=>{
    ...
    function topicItem(topic,marks,sec,title,kind,choice,filter){const q=take(poolTopic(topic,marks,filter),used,1,`${topic} ${marks}M`,errors)[0];if(!q)return;let or=null;if(choice){const p=poolTopic(topic,marks,filter).filter(x=>!used.has(key(x))&&!altUsed.has(key(x)));or=take(p,altUsed,1,`${topic} OR`,errors)[0]||null;}add(sec,title,marks,kind,q,or);}

    // I — Q1–6: grammar/vocabulary MCQs.
    for(let i=0;i<6;i++)topicItem('grammar',1,'I','Choose the correct alternative','mcq',false,isMCQ);
    // II — Q7–10: analogy MCQs. Analogy banks are explicitly classified as Analogy.
    ...
    // IV — Q14–20: chapter-wise 1M allocation in the model-paper order.
    ...
    // V — Q21–27: chapter-wise 2M allocation; Q27 carries the section choice.
    ...
    // VIII — Q37–40: one non-detail + three poetry, 3M RTC.
    ...
    // IX — Q41: 4M Quote from Memory. It must be a quote item, never a generic 4M long answer.
    ...
    // X — Q42–44: two prose + one poetry, each 4M with alternatives.
  ...
  function render(){
    ...
    let html='';
    for(let i=1;i<=10;i++)html+=`<button class="quick-card featured mock-paper-card" data-paper="${i}" type="button"><span>47Q</span><b>Mock Paper ${i}</b><small>47 questions • 100 marks • 3:15 hours • Exact paper order</small></button>`;
