...
(function(){
  ...
  async function openPreviousPaper(id,section,heading){
    try{
      const [paperRes,solutionRes]=await Promise.all([
      ...
      if(!paperRes.ok||!solutionRes.ok)throw new Error('Paper data unavailable');
      const data=await paperRes.json();
      const solutions=await solutionRes.json();
      const paper=data.papers.find(p=>p.id===id);const solved=solutions.papers[id];
      ...
      const sectionsHtml=paper.sections.map(s=>{
        ...
        const html=s.items.map((item,i)=>{
          ...
          return `<article class="previous-paper-item"><div class="pp-qno">Q${qno}</div><p class="pp-question">${escapeHtml(item)}</p>${ans?`<details class="pp-solution"><summary>View model answer${grammar?' + grammar help':''}</summary><div class="pp-answer"><strong>Model Answer:</strong> ${escapeHtml(ans)}</div>${grammar?`<div class="pp-grammar"><strong>Grammar Help:</strong> ${escapeHtml(grammar)}</div>`:''}</details>`:''}</article>`
        }).join('');
      ...
      view.innerHTML=`<div class="previous-paper-view"><div class="previous-paper-top"><p class="eyebrow">PREVIOUS YEAR QUESTION PAPER • SOLVED</p><h2>${escapeHtml(paper.title)}</h2><div class="previous-paper-meta"><span>15-E</span><span>${escapeHtml(paper.date)}</span><span>${paper.questions} Questions</span><span>${paper.marks} Marks</span><span>${escapeHtml(paper.duration)}</span><span>Model Answers</span></div></div>${sectionsHtml}<button class="secondary-btn previous-paper-back" type="button">← Back to Previous Papers</button></div>`;
